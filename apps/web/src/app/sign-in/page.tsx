...
export default function SignInPage() {
  ...
  const handleOAuth = async (fn: () => Promise<any>) => {
    try {
      await fn();
      toast.success('Signed in!');
      router.push('/chat');
      ...
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isSignUp ? 'Enter your email or use a provider' : 'Choose your sign-in method'}
          </p>
        </div>

        <div className="space-y-3">
          <button onClick={() => handleOAuth(signInWithGoogle)} type="button"
          ...
          <button onClick={() => handleOAuth(signInWithGithub)} type="button"
            className="flex w-full items-center justify-center gap-2 rounded-md border border-input bg-background px-4 py-2 text-sm font-medium hover:bg-accent">
